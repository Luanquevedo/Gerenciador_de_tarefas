...
const Task = () => {
  ...
  return (
    ...
    <div className="task__list">
      {tasks.map((task) => (
        <div key={task.id} className={`task__renderResult ${task.checked ? 'task__checked' : ''}`}>
          <li className="task__renderItem">
            {editingTaskId === task.id ? (
              ...
              <>
                <span style={{ color: task.checked ? 'green' : 'inherit' }}>{task.task}</span>
      ...
      <div className="task__renderList">
        ...
        />
        <button className="task__additem" onClick={handleAddTask}>
          <img id="cruz" src="" alt="" />
        </button>
        </div>
      </div>
